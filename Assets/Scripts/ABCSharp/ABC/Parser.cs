...
namespace ABC
{
    class Parser
    {
        ...
        /// <summary>
        /// Contributor: AorticTempo (Parker)
        /// Description: adds a function for getting the tempo.
        /// </summary>
        private void ParseTempo()
        {
            index += 2;
            var fullTempo = currentLine.Substring(index).Trim();
            string[] splittemp = fullTempo.Split('=');
            string[] fraction = splittemp[0].Split('/');
            float notelength = float.Parse(fraction[0]) / float.Parse(fraction[1]);
            float tempo = float.Parse(splittemp[1]);
            tune.tempo = (int?)tempo;
        }
